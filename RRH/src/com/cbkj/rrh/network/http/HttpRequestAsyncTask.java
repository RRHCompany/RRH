...
import com.cbkj.rrh.comment.utils.LogUtils;
import com.cbkj.rrh.comment.utils.SystemUtils;
import com.cbkj.rrh.network.http.HttpResponse.HttpTaskState;
import com.cbkj.rrh.view.RToast;
import com.cbkj.rrh.widget.dialog.LoadingProgress;
...
public class HttpRequestAsyncTask extends AsyncTask<Void, Void,HttpResponse> {
	...
	protected HttpResponse doInBackground(Void... params) {
		if(!SystemUtils.isNetworkConnect(context)){
	...
	protected void onPostExecute(HttpResponse response) {
		...
		switch (response.getState()) {
			...
			case STATE_NO_NETWORK_CONNECT:
				LoadingProgress.getInstance().dismiss();
				RToast.show(context, "网络未连接");
				break;
			case STATE_TIME_OUT:
				LoadingProgress.getInstance().dismiss();
				RToast.show(context, "连接超时");
				break;
			case STATE_UNKNOWN:
				LoadingProgress.getInstance().dismiss();
				RToast.show(context, "未知错误");
				break;
			case STATE_OK:
				break;
			default:
